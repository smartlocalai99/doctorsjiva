...
export async function loadCamps() {
  return requestJson('/api/camps', { cache: 'no-store' });
}

export async function saveCamp(input) {
  const result = await requestJson('/api/camps', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function loadReports() {
  const result = await requestJson('/api/reports', { cache: 'no-store' });
